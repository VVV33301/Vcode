...
class IdeWindow(QMainWindow):
    ...
    def program(self, code: EditorTab, debug: bool = False) -> None:
        ...
        if self.project:
            ...
            return
        with open(f'{CONFIG_PATH}/process_{tid}.txt', 'w') as f:
            f.write(command)
        if sys.platform == 'win32':
            with open(f'{CONFIG_PATH}/process_{tid}.bat', 'w', encoding='utf-8') as bat_win32:
                bat_win32.write(f'@echo off\nchcp 65001>nul\ncd {pth}\necho Interrupted > {fullpth}.output\n'
                                f'{command.format(filename=fullpth)}\necho Exit code: %errorlevel%\n'
                                f'echo %errorlevel% > {fullpth}.output\npause')
